...
우리는 Python의 `asyncio.gather` 패턴을 랭그래프의 노드 내부에 삽입하여 이 문제를 O(1)에 가깝게 최적화했습니다.
...
이 문제를 근본적으로 해결하기 위해, 각 워커(`run_single_worker`) 내부에 **단기 요약 담당 Sub-Agent(Map 작업자)**를 내장했습니다.
