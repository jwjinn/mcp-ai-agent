...
최신 SOTA 논문인 **LLMCompiler (UC Berkeley, 2023)**의 핵심 개념(Plan-Execute 동시 병렬성)을 차용하여, 우리는 Python의 `asyncio.gather` 패턴을 랭그래프의 노드 내부에 삽입하여 이 문제를 O(1)에 가깝게 최적화했습니다.
...
이 현상을 근본적으로 해결하기 위해, 우리는 사고(Reasoning)와 관찰(Observation)을 분리할 것을 주창한 **ReWOO (2023)** 논문의 아이디어를 도입했습니다.
