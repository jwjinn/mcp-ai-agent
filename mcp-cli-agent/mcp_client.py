...
class MCPClient:
    ...
    async def connect(self):
        """MCP 서버에 연결"""
        print(f"🔌 [{self.name}] 서버 연결 시도: {self.server_url} ...")
        try:
    ...
    async def refresh_tools(self):
        ...
        print(f"   └─ 🛠️  [{self.name}] 도구 {len(self.tools)}개 로드 완료")

    async def call_mcp_tool(self, name: str, arguments: dict) -> str:
        """도구 실행"""
        print(f"\n🚀 [{self.name}] Tool Call: {name} (Args: {arguments})")
        try:
            ...
            if len(final_output) > MAX_OUTPUT_LENGTH:
                ...
                print(f"✂️ [{self.name}] Truncation: 결과가 너무 길어 잘랐습니다. ({len(final_output)} chars)")

            # [변경] 디버깅을 위해 결과의 앞부분을 보여줌
            preview = final_output[:200].replace("\n", " ") + "..." if len(final_output) > 200 else final_output.replace("\n", " ")
            print(f"✅ [{self.name}] 성공 (Return: {preview})")
            return final_output
        except Exception as e:
            ...
            print(f"❌ [{self.name}] Error: {error_str}")
    ...
    async def cleanup(self):
        ...
        try:
            ...
            if "cancel scope" in str(e):
                ...
                print(f"⚠️ [{self.name}] Cleanup Warning: {e}")
        except Exception as e:
            print(f"⚠️ [{self.name}] Cleanup Error: {e}")
