...
from config import INSTRUCT_CONFIG, THINKING_CONFIG
...
def get_thinking_model(stream_prefix=""):
    ...
    callbacks = []
    if stream_prefix:
        print(f"\n{stream_prefix} ", end="") # 시작할 때 줄바꿈 및 프리픽스
...
def check_and_filter_duplicate_tools(history_messages: list, new_msg: BaseMessage) -> BaseMessage:
    ...
    for new_tc in new_msg.tool_calls:
        ...
        if not is_dup:
            ...
            print(f"🚫 [Loop Prevention] 연속/중복된 도구 실행 차단: {new_tc['name']}")
...
async def orchestrator_node(state: AgentState):
    ...
    print(f"🐛 [Debug] Orchestrator Raw Content:\n{content[:500]}...") # 디버깅용
    ...
    try:
        ...
        print(f"⚠️ [Orchestrator] JSON 파싱 실패: {e}")
    
    # [안전장치] 만약 파싱 실패하거나 계획이 비어있다면 -> K8s 전문가에게 전체 위임
    if not worker_plans:
        print("⚠️ [Orchestrator] 계획 수립 실패 또는 결과 없음 -> K8s Fallback 모드 작동")
        worker_plans = {
...
async def run_single_worker(worker_name: str, instruction: str, tools: list):
    ...
    print(f"👷 [{worker_name}] 시작: {instruction}")
    ...
    try:
        ...
        if response.tool_calls:
            ...
            for tc in response.tool_calls:
                ...
                if selected_tool:
                    print(f"   🔨 [{worker_name}] 도구 실행: {tc['name']}")
            ...
            **[작업 지시]**
            ...
            print(f"   📝 [{worker_name}] 도구 결과 요약 중... (Sub-Agent Summarization)")
...
async def synthesizer_node(state: AgentState):
    ...
    print(f"   📝 [Synthesizer] 각 전문가의 요약본 취합 완료 (총 길이: {len(worker_results_str)}자)")
    ...
    4. 결과에 실제 에러 문구(Unauthorized, Connection Refused 등)나 알 수 없는 크래시 흔적이 있을 때만 수동 점검을 제안하세요.
    """
